...
# Bering sea
proj = ccrs.LambertConformal(central_longitude = -170,
                             central_latitude = 65., cutoff = 40.)
ax  = plt.axes(projection = proj)
fig = plt.figure(figsize=(11,8.5))
ax  = fig.add_subplot(1, 1, 1, projection = proj)
ax.set_extent((-185, -145, 55, 75), crs=ccrs.PlateCarree())

#print(np.linspace(-180,180,37))
ax.gridlines(crs=ccrs.PlateCarree(),
      xlocs=np.linspace(-180,180,37),
      ylocs=[55, 60, 65, 70, 75, 80, 85, 90] )
#ax.gridlines(crs=ccrs.PlateCarree() )
ax.add_feature(cfeature.GSHHSFeature(levels = [1,2,3,4], scale = "low") )

ax.quiver(init_lon[:count], init_lat[:count], u[:count], v[:count], transform = ccrs.PlateCarree())
fig.savefig("bering.png")
plt.close()
