...
!RG: really initialize_io
  ...
  PRINT *,'time through xmetric set ',end_time - start_time
  start_time = end_time
...
!2ds_ice (not _prog or _diag)
  ...
  CALL cpu_time(end_time)
  PRINT *,'time for initial read ',end_time - start_time
  start_time = end_time
...
! Allocate varid_drift, initialize nvar_drift based on whether this is restart
  ...
  CALL cpu_time(end_time)
  PRINT *,'time for initialize_drifters ',end_time - start_time
  start_time = end_time

  ! For buoy output -- inlineable
  CALL initialize_out(outname, ncid_out, varid_out, nvar_out, nbuoys, dimids)
  CALL cpu_time(end_time)
  PRINT *,'time for initialize_out ',end_time - start_time
  start_time = end_time
  ...
  CALL cpu_time(end_time)
  PRINT *,'readin_drifters time ',end_time - start_time
...
! First/only time step (u,v, etc. in hand):
  ...
  CALL cpu_time(end_time)
  PRINT *,'run, write time ',end_time - start_time
